...
function render() {
    ...
    for (let i = 0; i < bundeslaender.length; i++) {
        ...
        <div id="landcontant${i}">
        <input id=input${i}> <button onclick="addContent(${i})">OK</button>
        </div>
        </div>
        `;

        let landcontant = document.getElementById(`landcontant${i}`);
        for (let j = 0; j < land['comments'].length; j++) {
            const comment = land['comments'][j];
            landcontant.innerHTML += `<div>${comment}</div>`;
        }
    }
}
